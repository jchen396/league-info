...
export default function Post({ match }) {
	...
	useEffect(() => {
		// selecting champion skins
		if (select) setClassList("skin-screen open");
		else setClassList("skin-screen");
	}, [select]);
	const infoArray = Object.values(info);
	const infoList = infoArray ? (
		infoArray.map((champ) => {
			return (
				<div className="main-display" key={champ.key}>
					<div className={classList}>
						<div className="skin-select">
							{champ.skins.map((skin, skinKey) => {
								return (
									<img
										key={skinKey}
										onClick={() => {
										...
										src={`http://ddragon.leagueoflegends.com/cdn/img/champion/loading/${champ.id}_${skin.num}.jpg`}
										alt="splash-art"
									/>
								);
							})}
					...
					<div className="main-image">
						...
						{/*BUTTON TO SWITCH SKINS*/}
						<img
							src={`http://ddragon.leagueoflegends.com/cdn/img/champion/loading/${champ.id}_${skin}.jpg`}
							alt="splash-art"
						/>
					</div>
					<div className="main-info">
						<div className="skill-set">
							{champ.spells.map((spell, spellKey) => {
								return (
	...
	if (infoList.every(checkEmpty)) {
		return <div className="center no-results">Loading . . .</div>;
	}
